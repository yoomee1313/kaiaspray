...
deploy()
{
  ...
  if [[ $HOMI_PATCH_ADDRESSBOOK_ADDR != "" ]]; then
    PATCHADDRESSBOOKADDR="--patch-address-book-addr $HOMI_PATCH_ADDRESSBOOK_ADDR"
  fi
  if [[ $HOMI_FUNDING_ADDR != "" ]]; then
    FUNDING_ADDR="--funding-addr $HOMI_FUNDING_ADDR"
  fi
  
  # compatibility options
  if [[ $HOMI_ISTANBUL_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    ISTANBUL_COMPATIBLE_BLOCKNUMBER="--istanbul-compatible-blocknumber $HOMI_ISTANBUL_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_LONDON_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    LONDON_COMPATIBLE_BLOCKNUMBER="--london-compatible-blocknumber $HOMI_LONDON_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_MAGMA_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    MAGMA_COMPATIBLE_BLOCKNUMBER="--magma-compatible-blocknumber $HOMI_MAGMA_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_SHANGHAI_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    SHANGHAI_COMPATIBLE_BLOCKNUMBER="--shanghai-compatible-blocknumber $HOMI_SHANGHAI_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_PRAGUE_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    PRAGUE_COMPATIBLE_BLOCKNUMBER="--prague-compatible-blocknumber $HOMI_PRAGUE_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_KORE_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    KORE_COMPATIBLE_BLOCKNUMBER="--kore-compatible-blocknumber $HOMI_KORE_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_KAIA_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    KAIA_COMPATIBLE_BLOCKNUMBER="--kaia-compatible-blocknumber $HOMI_KAIA_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_KIP103_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    KIP103_COMPATIBLE_BLOCKNUMBER="--kip103-compatible-blocknumber $HOMI_KIP103_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_KIP160_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    KIP160_COMPATIBLE_BLOCKNUMBER="--kip160-compatible-blocknumber $HOMI_KIP160_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_RANDAO_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    RANDAO_COMPATIBLE_BLOCKNUMBER="--randao-compatible-blocknumber $HOMI_RANDAO_COMPATIBLE_BLOCKNUMBER"
  fi
  if [[ $HOMI_CUNCUN_COMPATIBLE_BLOCKNUMBER != "" ]]; then
    CUNCUN_COMPATIBLE_BLOCKNUMBER="--cancun-compatible-blocknumber $HOMI_CUNCUN_COMPATIBLE_BLOCKNUMBER"
  fi

  ./homi setup --gen-type=local $HOMI_CONFIG $PATCHADDRESSBOOK $PATCHADDRESSBOOKADDR \
      $FUNDING_ADDR $REGISTRY_MOCK \
      $ISTANBUL_COMPATIBLE_BLOCKNUMBER $LONDON_COMPATIBLE_BLOCKNUMBER $MAGMA_COMPATIBLE_BLOCKNUMBER \
      $SHANGHAI_COMPATIBLE_BLOCKNUMBER $PRAGUE_COMPATIBLE_BLOCKNUMBER $KORE_COMPATIBLE_BLOCKNUMBER \
      $KAIA_COMPATIBLE_BLOCKNUMBER $KIP103_COMPATIBLE_BLOCKNUMBER $KIP160_COMPATIBLE_BLOCKNUMBER \
      $RANDAO_COMPATIBLE_BLOCKNUMBER $CUNCUN_COMPATIBLE_BLOCKNUMBER \
  ...
  if [[ $HOMI_NUMOF_INITIAL_CN_NUM != 0 ]]; then
    ./homi setup --gen-type=local $HOMI_CONFIG $NODEKEYSDIR $PATCHADDRESSBOOK $PATCHADDRESSBOOKADDR \
        $FUNDING_ADDR $REGISTRY_MOCK \
        $ISTANBUL_COMPATIBLE_BLOCKNUMBER $LONDON_COMPATIBLE_BLOCKNUMBER $MAGMA_COMPATIBLE_BLOCKNUMBER \
        $SHANGHAI_COMPATIBLE_BLOCKNUMBER $PRAGUE_COMPATIBLE_BLOCKNUMBER $KORE_COMPATIBLE_BLOCKNUMBER \
        $KAIA_COMPATIBLE_BLOCKNUMBER $KIP103_COMPATIBLE_BLOCKNUMBER $KIP160_COMPATIBLE_BLOCKNUMBER \
